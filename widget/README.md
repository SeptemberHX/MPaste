...
- `ScrollItemsWidget` now triggers a save-directory migration step before loading items, so existing `.mpaste` files are upgraded to v3 on next app startup.
- `ClipboardItemInnerWidget` keeps the original layout, but the top header now uses a softer left-to-right tinted gradient, still brighter on the left and darker on the right.
